...
class CalculatorViewModel: ViewModel() {
    ...
    private fun negate() {
        ...
        if (resultFromLastOperation) {
            ...
            /*val alreadyNegative = state.number1.contains("-")
            val negatedValue = if (alreadyNegative)
                state.number1.drop(1) else "-" + state.number1*/
    ...
    private fun dropZerosAfterDecimal(number: Double): String {
        return if (isWholeNumber(number)){
            number.format(0) //formatted without decimal
        }else {
